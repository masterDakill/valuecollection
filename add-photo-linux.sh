#!/bin/bash
# Script ultra-simplifié: Ajoute une photo (HEIC/JPG) à votre collection (Linux)
# Gère automatiquement la conversion et compression
# Usage: ./add-photo-linux.sh photo.heic
...
for INPUT_FILE in "$@"; do
    ...
    if [[ "$INPUT_FILE" =~ \.(heic|HEIC|heif|HEIF)$ ]]; then
        ...
        convert "$INPUT_FILE" -quality 40 "$TEMP_JPG" >/dev/null 2>&1
    else
    ...
    if [ ! -f "$TEMP_JPG" ]; then
        echo "❌ Erreur de conversion/compression"
        continue
    fi
